...
public class GitOperations {
	...
	public RuleMetadata addFileToRepo(Customer cust, ICERuleResponse rRes) {
		try {
			...
			addNewCommitIdtoBucket(metaData, ruleFile);
			commit(git, cust);
	...
	private ByteArrayOutputStream iterateTreeTogetFileContents(TreeWalk treeWalk, String filename) {
		...
		try {
			...
			while (treeWalk.next()) {
				...
				// compare treeWalk path to file directory and name- ex.
				// "Torana/v1/InputView.js"
	...
	private void addNewCommitIdtoBucket(File file, File ruleFile)
			throws MissingObjectException, IncorrectObjectTypeException, IOException {
		String latestCommitId = getLatestCommitId();

		loadBucket(latestCommitId, ruleFile);

		// add to file
		FileOperations.getInstance().writeToMetaDataFile(ruleBucket.getRuleMetaData(), file);
	}

	private void loadBucket(String latestCommitId, File ruleFile) {
